...
MODULE(io_stream_ipv6);

class io_stream_ipv6_t : public io_stream_t {
...
public:
	...
	inline io_stream_ipv6_t(string_t const &name, config_t const &config) :
		io_stream_t(name, config), netaddr(config.address, config.port),
		defer_accept(config.defer_accept), ctl_tcp(config.cork) { }

	inline ~io_stream_ipv6_t() throw() { }
};

netaddr_t const &io_stream_ipv6_t::bind_addr() const throw() {
	return netaddr;
}

netaddr_t *io_stream_ipv6_t::new_netaddr() const {
	return new netaddr_ipv6_t;
}

void io_stream_ipv6_t::fd_setup(int fd) const {
	if(defer_accept) {
		int i = 1;
...
fd_ctl_t const *io_stream_ipv6_t::ctl() const {
	return &ctl_tcp;
}

namespace io_stream_ipv6 {
