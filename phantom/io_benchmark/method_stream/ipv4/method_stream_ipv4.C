...
struct method_stream_ipv4_t : public method_stream_t {
	...
	public:
		inline locals_t(config::list_t<address_ipv4_t> const &bind) :
			...
			vals = new netaddr_ipv4_t[count];

			for(typeof(bind.ptr()) ptr = bind; ptr; ++ptr)
				vals[ind++] = netaddr_ipv4_t(ptr.val(), 0);
		}
...
config_binding_parent(method_stream_ipv4_t, method_stream_t, 1);
