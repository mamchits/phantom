// This file is part of the phantom::io_benchmark::method_stream::ipv6 module.
...
MODULE(io_benchmark_method_stream_ipv6);

struct method_stream_ipv6_t : public method_stream_t {
	...
	public:
		inline locals_t(config::list_t<address_ipv6_t> const &bind) :
			spinlock(), vals(NULL), count(0), ind(0) {

			for(typeof(bind.ptr()) ptr = bind; ptr; ++ptr)
				++count;

			assert(count > 0);

			vals = new netaddr_ipv6_t[count];

			for(typeof(bind.ptr()) ptr = bind; ptr; ++ptr)
				vals[ind++] = netaddr_ipv6_t(ptr.val(), 0);
		}

		inline ~locals_t() throw() { delete [] vals; }
		...
		_retry:
			...
			if(::bind(fd, addr.sa, addr.sa_len) < 0) {
				if(errno == EADDRNOTAVAIL && ++r < count)
					goto _retry;
...
public:
	struct config_t : method_stream_t::config_t {
		...
		uint16_t port;
		config::list_t<address_ipv6_t> bind;
	...
	inline method_stream_ipv6_t(string_t const &name, config_t const &config) :
	...
	inline ~method_stream_ipv6_t() throw() {
		if(locals) delete locals;
	}
};

netaddr_t const &method_stream_ipv6_t::dest_addr() const {
	return netaddr;
}

void method_stream_ipv6_t::bind(int fd) const {
	if(locals)
		locals->bind(fd);
}

namespace method_stream_ipv6 {
config_binding_sname(method_stream_ipv6_t);
config_binding_value(method_stream_ipv6_t, address);
config_binding_value(method_stream_ipv6_t, port);
config_binding_value(method_stream_ipv6_t, bind);
config_binding_value(method_stream_ipv6_t, cork);
config_binding_parent(method_stream_ipv6_t, method_stream_t, 1);
config_binding_ctor(method_t, method_stream_ipv6_t);
}
