...
struct method_stream_ipv6_t : public method_stream_t {
	...
	public:
		inline locals_t(config::list_t<address_ipv6_t> const &list) :
			spinlock(), ind(0), addrs(
				list,
				[](address_ipv6_t const &addr) { return netaddr_ipv6_t(addr, 0); }
			) { }
...
config_binding_parent(method_stream_ipv6_t, method_stream_t);
config_binding_cast(method_stream_ipv6_t, method_t);
