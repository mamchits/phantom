...
class logger_brief_t : public logger_t, io_logger_file_t {
...
public:
	struct config_t : logger_t::config_t, io_logger_file_t::config_t {
		...
		inline config_t() throw() :
			logger_t::config_t(),
			io_logger_file_t::config_t(), time_format(native) { }
...
config_binding_parent(logger_brief_t, logger_t, 1);
config_binding_parent(logger_brief_t, io_logger_file_t, 2);
...
void logger_brief_t::commit(
	...
	size_t size = ({
		...
		if(time_format == native) {
			...
			interval_t t = res.time_start - timeval_unix_origin;
			if(t < interval_zero) t = interval_zero; // Don't wanna play with sign

			out
				.print(t / interval_second)('.')
				.print((t % interval_second) / interval_millisecond, "03")('\t')
			;
		}
		out
			(tag)('\t')
			.print((res.time_end - res.time_start) / interval_microsecond)('\t')
			.print((res.time_conn - res.time_start) / interval_microsecond)('\t')
			.print((res.time_send - res.time_conn) / interval_microsecond)('\t')
			.print((res.time_recv - res.time_send) / interval_microsecond)('\t')
			.print((res.time_end - res.time_recv) / interval_microsecond)('\t')
